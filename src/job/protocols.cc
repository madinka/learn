...
BOOL IProtocol::send(_FormatRS485Data* pDataRS485, void (*printTx_)(UCHAR*, int)) {
  ...
  const unsigned char* pData = pDataRS485->pDataMessage;
...
ErrCode IProtocol::resetLayerParameters( Settings comPortName ) {
  ...
  string stringPortName = comPortName;//simple_type_processors::CString2string(comPortName);
...
int ProtocolsLayerImpl::Send(const unsigned char type_message,
  ...
  bool result = old_protocol_->send(&request, printRx);
  return result;
}

int ProtocolsLayerImpl::Receive(unsigned char& type_message,
                                std::vector<unsigned char> & data) {
  FormatDataToSend response = {0x00, 0x00, 0x00, &data[0]};
  BOOL receiveRequest = old_protocol_->receive(&response, &protocols::printRx);
  type_message = response.TypeMessage;

  // Обжимаем пакет
  data.resize(response.LengthDataMessage);
  return receiveRequest;
}

protocols::ReadResult ProtocolsLayerImpl::ReceivePkg() {
  ReadResult result;
  result.data.resize(512, 0);
  bool success = this->Receive(result.type_message, result.data);
